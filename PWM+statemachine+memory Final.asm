...
mov pwm, #100
clr TR2
mov sec_count, #0		;the sec_count is set to 0 and timer is off in state 1. The timer will start as soon as we move to state 2
Set_Cursor(2, 1)
...
jnb START_STOP, stop		;checking if stop button is pressed
mov a, time_soak
clr c
subb a, sec_count		;time_soak and sec are compared, if time_soak>=sec, it will remain in state2, else go to state3
jnc state2_done
mov state, #3
state2_done:
ljmp forever
