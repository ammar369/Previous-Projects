/*
 File:              mazesolver.c
 Purpose:           Lab 4 Take Home
 Author:			Ammar Rehan & Cam Sparkes
 Student #s:		10649151 and 35015916
 CS Accounts:		f7v0b and p1c2b
...
int main(void)
{
	...
	generate_all_paths(maze, dimension, 0, 0, ""); //initail row, cloumn and path values are 0 or NULL
...
void generate_all_paths(maze_cell ** maze, int dimension, int row, int column, char * path)
{
	...
	if ( // INSERT CODE HERE: Simply return if we hit one of the base cases (there are more than 1)
		...
		((row<0) || (column<0) || (row>=dimension) || (column>=dimension)) ||//exits if coordinate is out of bounds
		(maze[row][column].visited== VISITED) ||
		(maze[row][column].character == MAZE_WALL)) { //check if coordinate is a wall
		
		return; //exits in the case it is any of the above
	}
	...
	/* Otherwise deals with the recursive case.  Pushes the current coordinate onto the path
	   ...
	   ELSE the current location is marked as used and the search continues
		...
		if (column == (dimension - 1)) {
			/* 1. Reallocate memory in global array to make room
			...
			maze[row][column].visited = VISITED; //set as visited first
			 generate_all_paths(maze, dimension, row - 1, column, new_path); //check for above
			 generate_all_paths(maze, dimension, row, column + 1, new_path); //check for right
			 generate_all_paths(maze, dimension, row + 1, column, new_path); //check for below
			 generate_all_paths(maze, dimension, row, column - 1, new_path); //check for left
			maze[row][column].visited = UNVISITED; //set as unvisited last
			return;
		}
	}
}
...
int path_cost(char * path_string)
{
	...
	char j = *path_string;
	while (j != ('0')) {
		i = j - '0'; //this converts the number from char to int and i gets that int value
		sum = sum + i; //sum is updated to the new value
		count++; //this tell how many times loop has run, so *path_string is not lost
		j = *(path_string + count); //j goes to next char in path_string, skipping those already read
	}
}
...
/*
Prints the shortest and cheapest path
*/
